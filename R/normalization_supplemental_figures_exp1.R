...
### grab the 3 sets.
set = exp1.edaseq$set
dataWithin = exp1.edaseq$dataWithin
dataNorm = exp1.edaseq$dataNorm

### set up plotting objects.
colors = brewer.pal(8, "Dark2")[c(1,2)]
legend.cols = colors
group_color = pData(set)$DN
...
################################################# PLOTTING.
#svglite("figures/edaseq_exp1.svg", height=16, width=18, system_fonts = list(sans = "Arial"))
cex.lab = 1.75
cex.main = 2
cex.axis=1.3
...
if(use_3d_pca_plots == FALSE) {
    plotPCA(set, col=plot.cols, pch=16, xlim=c(-0.6, 0.6), ylim=c(-0.6,0.6), main="PCA plot", labels=FALSE)
    legend("topright", legend=levels(group_color), fill=legend.cols, cex=1.1, bty="n")
...
tt = exp1.results$W0$top
tt.ruv = exp1.results$W6$top
set = exp1.results$W0$set
set.ruv = exp1.results$W6$set
...
########## save plot as SVG.
#svglite("figures/ruvseq_exp1.svg", height=20, width=12, system_fonts = list(sans = "Arial"))
